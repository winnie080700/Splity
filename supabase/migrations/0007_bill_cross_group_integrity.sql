...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM jsonb_to_recordset(COALESCE(p_input->'payment_contributions', '[]'::JSONB)) AS x(participant_id UUID)
    WHERE NOT EXISTS (
      SELECT 1 FROM public.participants p WHERE p.id = x.participant_id AND p.group_id = p_group_id
    )
  ) THEN
    RAISE EXCEPTION 'payment contribution participant is not in group %', p_group_id USING ERRCODE = '23514';
  END IF;
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'payment contribution participant is not in group %', v_target_group_id USING ERRCODE = '23514';
  END IF;
