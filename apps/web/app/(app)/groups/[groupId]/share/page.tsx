  ...
  return (
    <div className="grid gap-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-zinc-200 pb-5">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Public share link</h1>
          <p className="mt-2 text-sm text-zinc-500">{group.name}</p>
        </div>
        <Badge tone={activeShare ? "green" : "neutral"}>{activeShare ? "Active" : "No active share"}</Badge>
      </header>

      {activeShare && publicUrl ? (
        <section className="grid gap-2 rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-semibold">Current public URL</h2>
          <div className="break-all rounded-md bg-zinc-50 px-3 py-2 text-sm text-zinc-700">{publicUrl}</div>
          <p className="text-sm text-zinc-500">
            Generated {new Date(activeShare.createdAtUtc).toLocaleString()}
          </p>
        </section>
      ) : null}
      ...
      {!canGenerate ? (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          New public share links can only be generated while the group is settling. Existing active links can still be deactivated.
        </div>
      ) : null}
