...
public static class DatabaseInitializer
{
    public static async Task InitializeAsync(SplityDbContext dbContext, CancellationToken cancellationToken = default)
    {
        ...
        await EnsureParticipantSchemaAsync(dbContext, cancellationToken);
        await EnsureBillSchemaAsync(dbContext, cancellationToken);
    ...
    private static async Task EnsureParticipantSchemaAsync(
        SplityDbContext dbContext,
        CancellationToken cancellationToken)
    {
        if (dbContext.Database.IsInMemory())
        ...
        try
        {
            if (!await ColumnExistsAsync(connection, connection.Database, "participants", "username", cancellationToken))
            {
                await using var command = connection.CreateCommand();
    ...
    private static async Task EnsureBillSchemaAsync(
        SplityDbContext dbContext,
        CancellationToken cancellationToken)
    {
        if (dbContext.Database.IsInMemory())
        ...
        try
        {
            if (!await ColumnExistsAsync(connection, connection.Database, "bills", "reference_image_data_url", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE bills ADD COLUMN reference_image_data_url LONGTEXT NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }
        }
        finally
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "clerk_user_id", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN clerk_user_id VARCHAR(100) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }
            ...
            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "username", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN username VARCHAR(50) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "default_payment_payee_name", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN default_payment_payee_name VARCHAR(150) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "default_payment_method", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN default_payment_method VARCHAR(120) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "default_payment_account_name", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN default_payment_account_name VARCHAR(150) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "default_payment_account_number", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN default_payment_account_number VARCHAR(120) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "default_payment_notes", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN default_payment_notes VARCHAR(2000) NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await ColumnExistsAsync(connection, connection.Database, "app_users", "default_payment_qr_data_url", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "ALTER TABLE app_users ADD COLUMN default_payment_qr_data_url LONGTEXT NULL";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await IndexOnColumnExistsAsync(connection, connection.Database, "app_users", "username", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "CREATE UNIQUE INDEX ux_app_users_username ON app_users (username)";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }

            if (!await IndexOnColumnExistsAsync(connection, connection.Database, "app_users", "clerk_user_id", cancellationToken))
            {
                await using var command = connection.CreateCommand();
                command.CommandText = "CREATE UNIQUE INDEX ux_app_users_clerk_user_id ON app_users (clerk_user_id)";
                await command.ExecuteNonQueryAsync(cancellationToken);
            }
        }
        finally
