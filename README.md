...
当前实现已经围绕真实使用场景做过一轮整理：

- 未登录访问 `/` 进入 Home
- 已登录且本地会话有效时访问 `/` 自动进入 Dashboard
- 支持 `zh / en` 全局语言切换，入口统一放在 footer
- 群组状态统一为 `未落实 / 结算中 / 已结算`
- `结算中 / 已结算` 下 Participants、Bills、Settlement 都是只读
- Forgot Password 已接入 SMTP 临时密码邮件
- Share bill modal 已改成 step flow，并保存固定分享链接与 receiver 付款信息
...
- 登录 / 注册共用页面
- 支持 Remember me
- Forgot Password 支持输入邮箱、统一成功提示、60 秒重发倒计时
...
- 数据库初始化只负责建表和补充缺失字段，不会自动写入示例群组
- 公开分享页路径为 `/s/:shareToken`
- 当前仓库没有引入重型 UI 框架替换现有系统
- footer 语言切换和 share bill 流程都已经走当前真实实现，不再依赖旧占位逻辑
